...
public:
	...
	public:
		Node() :p(N),axis(-1) {}
		Node(const std::vector<float>& _p, const T& _data) : p(_p), d(_data),axis(-1) { if (_p.size()!=N) throw("Wrong dimension number in kd-tree"); }
...
public:
	...
	void store(const std::vector<float>& point, const T& data) { nodes.push_back(Node(point,data)); }
private:
	static void median_split(std::vector<Node>& p, int start, int end, int median, int axis);
	static void balanceSegment(std::vector<Node>& pbal, std::vector<Node>& porg, int index, int start, int end, const std::vector<float>& bbmin, const std::vector<float>& bbmax);
...
private:
	...
	void find(const std::vector<float>& p, int index, float radius, list<const Node*> &nodes) const;
	void find(const std::vector<float>& p, int index, int nb_elements, float &dist_worst, std::vector<const Node*> &nodes, std::vector<pair<int,float> > &dist) const;
...
public:
	//========================================================================================================
	// Fixed Radius
	int find(const std::vector<float>& p, float radius, list<const Node*>* nodes) const
	{
		if (nodes)
		{
	...
	void find(const std::vector<float>& p, int nb_elements, std::vector<const Node*> &nodes, float &max_distance) const
	{
		nodes.clear(); 
...
//Private Find(radius)
template <class T, unsigned int N >
void KDTree<T,N>::find(const std::vector<float>& p, int index, float radius, list<const Node*>& nodes) const
	{ 
...
template <class T, unsigned int N>
void KDTree<T,N>::find(const std::vector<float>& p, int index, int nb_elements, float &dist_worst, 
					   std::vector<const Node*> &nodes, std::vector<pair<int,float> > &dist) const
	{ 
		float aux;
...
// Closest
template <class T, unsigned int N >
int KDTree<T,N>::closest(const std::vector<float>& p, int index, int best) const
	{ 
		int sol=best;
...
template <class T, unsigned int N >
void KDTree<T,N>::balanceSegment(std::vector<Node>& pbal, std::vector<Node>& porg, int index, int start, int end, const std::vector<float>& bbmin, const std::vector<float>& bbmax)
	{
		int median=1;
