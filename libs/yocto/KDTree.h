...
(Computer Graphics) tought at Universidad de Zaragoza (Spain). As such, it does
not intend to be fast or general, but just to provide an educational tool for
undergraduate students. This software is provided as is, and any express or
implied warranties are disclaimed. In no event shall copyright holders be liable
for any damage.
**********************************************************************************/
#ifndef _KDTREE_H_
#define _KDTREE_H_
...
/*********************************************************************************
This code has been adapted from Adolfo Munoz's Mjolnir-RT, developed at
Universidad de Zaragoza (Spain). Particularly, the functions to find the
N-closest nearest neighbors have been added to the original code.
**********************************************************************************/
#include <math.h>
...
class KDTree {
 public:
  class Node {
   ...
   public:
    Node() : p(N), axis(-1) {}
    Node(const std::vector<float>& _p, const T& _data)
        : p(_p), d(_data), axis(-1) {
      if (_p.size() != N) throw("Wrong dimension number in kd-tree");
    }
    const std::vector<float>& point() const { return p; }
 ...
 public:
  KDTree() {}
  int  size() { return balanced.size(); }
  void clear() {
    nodes.clear();
    balanced.clear();
  }
  void store(const std::vector<float>& point, const T& data) {
    nodes.push_back(Node(point, data));
  }

 private:
  static void median_split(
      std::vector<Node>& p, int start, int end, int median, int axis);
  static void balanceSegment(std::vector<Node>& pbal, std::vector<Node>& porg,
      int index, int start, int end, const std::vector<float>& bbmin,
      const std::vector<float>& bbmax);
 ...
 private:
  int  closest(const std::vector<float>& p, int index, int best) const;
  void find(const std::vector<float>& p, int index, float radius,
      list<const Node*>& nodes) const;
  void find(const std::vector<float>& p, int index, int nb_elements,
      float& dist_worst, std::vector<const Node*>& nodes,
  ...
  class HeapComparison {
   public:
    bool operator()(
        const pair<int, float>& val1, const pair<int, float>& val2) const {
      return val1.second < val2.second;
    }
  };
  void update_heap_nodes(const Node& node, const float distance,
      int nb_elements, std::vector<const Node*>& nodes,
      std::vector<pair<int, float>>& dist) const;

 public:
  //========================================================================================================
  // Fixed Radius
  int find(const std::vector<float>& p, float radius,
      list<const Node*>* nodes) const {
    if (nodes != nullptr) {
      ...
      list<const Node*> local_nodes = list<const Node*>();
  ...
  void find(const std::vector<float>& p, int nb_elements,
      std::vector<const Node*>& nodes, float& max_distance) const {
    nodes.clear();
  ...
  const Node& find(const std::vector<float>& p) const {
...
// Private Find(radius)
template <class T, unsigned int N>
void KDTree<T, N>::find(const std::vector<float>& p, int index, float radius,
    list<const Node*>& nodes) const {
  ...
  if (index < ((balanced.size() - 1) / 2)) {
    float distaxis = p[balanced[index].axis] -
                     balanced[index].point()[balanced[index].axis];
    if (distaxis < 0.0)  // left node first
    {
      find(p, 2 * index, radius, nodes);
      if (radius >
          fabs(distaxis))  // Maybe we can find more nodes on the other child
...
template <class T, unsigned int N>
void KDTree<T, N>::update_heap_nodes(const Node& node, const float distance,
    int nb_elements, std::vector<const Node*>& nodes,
    std::vector<pair<int, float>>& dist) const {
...
template <class T, unsigned int N>
void KDTree<T, N>::find(const std::vector<float>& p, int index, int nb_elements,
    float& dist_worst, std::vector<const Node*>& nodes,
    std::vector<pair<int, float>>& dist) const {
  float aux;
...
// Closest
template <class T, unsigned int N>
int KDTree<T, N>::closest(
    const std::vector<float>& p, int index, int best) const {
  int   sol      = best;
  ...
  if (index < ((balanced.size() - 1) / 2)) {
    ...
    if (distaxis < 0.0)  // left node first
    {
      ...
      if (distbest >
          fabs(distaxis))  // Maybe the best solution is on the other side
      {
...
template <class T, unsigned int N>
void KDTree<T, N>::balanceSegment(std::vector<Node>& pbal,
    std::vector<Node>& porg, int index, int start, int end,
    const std::vector<float>& bbmin, const std::vector<float>& bbmax) {
  int median = 1;
  ...
  int axis = 0;
  for (int i = 1; i < N; i++)
    if ((bbmax[i] - bbmin[i]) > (bbmax[axis] - bbmin[axis])) axis = i;
  ...
  if (median > start) {
    ...
    if (start < median - 1) {
      std::vector<float> newbbmax = bbmax;
  ...
  if (median < end) {
    ...
    if (median + 1 < end) {
      std::vector<float> newbbmin = bbmin;
      newbbmin[axis]              = pbal[index].point()[axis];
      balanceSegment(
          pbal, porg, 2 * index + 1, median + 1, end, newbbmin, bbmax);
    } else {
...
void KDTree<T, N>::balance() {
  {
    ...
    int                i;
    std::vector<float> bbmax = nodes.front().point();
    std::vector<float> bbmin = nodes.front().point();
    ...
    KDTree<T, N>::balanceSegment(
        balanced, aux, 1, 1, balanced.size() - 1, bbmin, bbmax);
  }
}
}  // namespace yocto
